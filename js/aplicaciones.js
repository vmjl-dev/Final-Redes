document.addEventListener('DOMContentLoaded', function() {
            ...
            // Animación de tarjetas de aplicación
            const appCards = document.querySelectorAll('.application-card');
            appCards.forEach(card => {
                card.addEventListener('mouseenter', () => {
                    card.style.animation = 'cardPulse 1.5s infinite';
                });
                ...
                const cardKeyframes = `
                    @keyframes cardPulse {
                        0% { box-shadow: 0 0 10px rgba(0, 217, 166, 0.3); }
                        50% { box-shadow: 0 0 25px rgba(0, 217, 166, 0.6); }
                        100% { box-shadow: 0 0 10px rgba(0, 217, 166, 0.3); }
                    }
                `;
                
                styleSheet.insertRule(cardKeyframes, styleSheet.cssRules.length);
            });
        });
