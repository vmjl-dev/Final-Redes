document.addEventListener('DOMContentLoaded', function() {
            ...
            // Animación de núcleo de fibra
            const core = document.querySelector('.core');
            core.style.animation = 'corePulse 4s infinite';
            
            const coreKeyframes = `
                @keyframes corePulse {
                    0% { box-shadow: 0 0 10px rgba(0, 217, 166, 0.5); }
                    50% { box-shadow: 0 0 25px rgba(0, 217, 166, 0.8); }
                    100% { box-shadow: 0 0 10px rgba(0, 217, 166, 0.5); }
                }
            `;
            
            styleSheet.insertRule(coreKeyframes, styleSheet.cssRules.length);
        });
