document.addEventListener('DOMContentLoaded', function() {
            ...
            // Efecto de aparición gradual para los elementos de contenido
            const contentElements = document.querySelectorAll('.section-title, .content-text, .conclusion-block, .future-card');
            ...
            title.style.animation = 'pulseTitle 3s infinite';
            
            const titleKeyframes = `
                @keyframes pulseTitle {
                    0% { text-shadow: 0 2px 10px rgba(0, 180, 150, 0.4); }
                    50% { text-shadow: 0 2px 20px rgba(0, 217, 166, 0.8); }
                    100% { text-shadow: 0 2px 10px rgba(0, 180, 150, 0.4); }
                }
            `;
            
            styleSheet.insertRule(titleKeyframes, styleSheet.cssRules.length);
        });
